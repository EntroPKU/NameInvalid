...
/************************** Functions/APIs *****************************/
  ...
  bool mygraph_load_elf(const std::string& work_path, std::vector<std::string>& elfInfoPath)
  {
	#ifdef __PS_INIT_AIE__

	std::string work_dir = (work_path.empty() ?  "work" : work_path);
	printf("Loading elfs of graph mygraph...\n");
  ...
  void mygraph_init(const std::string& work_path)
  {

	printf("Initializing graph mygraph...\n");
    #ifdef __PS_INIT_AIE__

	std::string work_dir = (work_path.empty() ?  "work" : work_path);
	// S_MM2S_DMA_ch0_C24_R0 M_SOUTH_ch3_C24_R0 net1

	XAie_StrmConnCctEnable(&DevInst, XAie_TileLoc(24, 1), DMA, 0, SOUTH, 3);

	// S_SHIM_NORTH_ch3_C24 M_SHIM_SOUTH_ch3_C24 net1

	XAie_StrmConnCctEnable(&DevInst, XAie_TileLoc(24, 0), NORTH, 3, SOUTH, 3);

	// S_SHIM_SOUTH_ch4_C24 M_SHIM_NORTH_ch5_C24 net0

	XAie_StrmConnCctEnable(&DevInst, XAie_TileLoc(24, 0), SOUTH, 4, NORTH, 5);
	...
	printf("Resetting cores of graph mygraph...\n");
	...
	printf("Configuring DMAs of graph mygraph...\n");

	//Setting buffer buf0 of mygraph.filer_singal_kernal.in[0]
	{
	...
	//Setting buffer buf0d of mygraph.filer_singal_kernal.in[0]
	{
		XAie_DmaDesc DmaInst;
		XAie_DmaDescInit(&DevInst, &DmaInst, XAie_TileLoc(24, 2));
		XAie_DmaSetAddrLen(&DmaInst, 0x6000, 6144);
	...
	//Setting buffer buf1 of mygraph.filer_singal_kernal.out[0]
	{
		XAie_DmaDesc DmaInst;
		XAie_DmaDescInit(&DevInst, &DmaInst, XAie_TileLoc(24, 1));
		XAie_DmaSetAddrLen(&DmaInst, 0x4000, 2048);
	...
	//Setting buffer buf1d of mygraph.filer_singal_kernal.out[0]
	{
		XAie_DmaDesc DmaInst;
		XAie_DmaDescInit(&DevInst, &DmaInst, XAie_TileLoc(24, 1));
		XAie_DmaSetAddrLen(&DmaInst, 0x0, 2048);
	...
	XAie_DmaChannelPushBdToQueue(&DevInst, XAie_TileLoc(24, 1), 0, DMA_MM2S, 0);
	XAie_DmaChannelEnable(&DevInst, XAie_TileLoc(24, 1), 0, DMA_MM2S);

	printf("Configuring PL-Interface for graph mygraph...\n");
	XAie_PlIfBliBypassDisable(&DevInst, XAie_TileLoc(24, 0), 4);
	XAie_PlToAieIntfEnable (&DevInst, XAie_TileLoc(24, 0), 4, PLIF_WIDTH_32);
	XAie_AieToPlIntfEnable (&DevInst, XAie_TileLoc(24, 0), 3, PLIF_WIDTH_32);
 ...
 void mygraph_core_debug_halt()
  {
	printf("Putting core(s) of graph mygraph on halt state for debugging...\n");
	XAie_CoreDebugHalt(&DevInst, XAie_TileLoc(24, 1));
	return;
  }

  void mygraph_core_enable()
  {
	printf("Enabling core(s) of graph mygraph...\n");
...
/************************** Clock gating *****************************/
  ...
  {
    XAie_LocType locs[4] = {XAie_TileLoc(24, 1), XAie_TileLoc(24, 2), XAie_TileLoc(19, 0), XAie_TileLoc(24, 0), };
    XAie_PmRequestTiles(&DevInst, locs, 4);
...
/************************** Graph Configurations  *****************************/
  ...
  //{id, name, graphLoadElfFunc, graphInitFunc, graphDebugHalt, coreColumns, coreRows, iterMemColumns, iterMemRows, iterMemAddrs, triggered, plKernelInstanceNames, plAxiLiteModes, plDriverStartFuncs, plDriverCheckIPDoneFuncs}
    {0, "mygraph", &mygraph_load_elf, &mygraph_init, &mygraph_core_debug_halt, {24}, {0}, {24}, {1}, {0x4}, {0}, {}, {}, {}, {},  }, 
  };
